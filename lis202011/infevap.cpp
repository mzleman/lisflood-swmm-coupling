...
void FlowDirDEM(Pars *Parptr, Arrays *Arrptr, States *Statesptr, BoundCs *BCptr)
{
		...
		for(i=0;i<Parptr->xsz;i++) 
		{
			...
			if(Arrptr->DEM[p0]!=1e10)
			{
				...
				if(north<=east && north<=south && north<=west && north<Arrptr->DEM[p0]) // flow to the north
				{
					...
					Arrptr->RouteInt[p0]=dy/Routing_Speed; // calculate RouteInt 计算时间
				}
	...
	{
		Arrptr->Route_dH=new double[Parptr->xsz*Parptr->ysz]();//初始化每个值为0.0
		return;
	}
}
//-----------------------------------------------------------------------------
...
void Routing(States *Statesptr, Pars *Parptr, Solver *Solverptr, Arrays *Arrptr)
{
	...
	for(j=0;j<Parptr->ysz;j++) 
	{
		...
		{		
          ...
          //以下代码可以改为
          /*
          p1=Arrptr->FlowDir[p0];
          z1=Arrptr->DEM[p1]; 
					h1=Arrptr->H[p1];
          dh=z0+h0-z1-h1;
          Sf=dh/Parptr->dx 这里不加绝对值，就可以排除flow<0的情况
          if(h0<Solverptr->DepthThresh || Sf>=Parptr->RouteSfThresh)
          {....Perform routing...}
          */
            ...
            /*
            flow的计算方式不太对，因为如果是楼房建筑物等边界，楼顶水位远高于地面水位，会导致计算的flow过大。
            改进:
            flow=z1+h1>z0?(z0+h0-z1-h1):h0;
            */
            ...
            //计算时间因子，Tstep/RouteInterval，RouteInterval=dx/Routing_Speed
						flow_fraction=Solverptr->Tstep/Arrptr->RouteInt[p0]; // fraction of cell volume to route in this time step
						if(flow_fraction>1) flow_fraction=1; // prevent flow fraction>1 (should never happen!) 如果时间因子大于1，则会导致排水量大于容积差

						flow=flow*flow_fraction; // calc correct H to route this timestep

						Arrptr->Route_dH[p0]-=flow; // Record change in H in source cell flow为正。每个栅格既可能是排出点，也可以是接收点。最多排出1次，可能接收多次。
						Arrptr->Route_dH[Arrptr->FlowDir[p0]]+=flow; // Record change in H in recipient cell 累加多次
            /*
            最后应该补充Vx和Vy的更新操作，
            if(p0-p1==-1),往西流动,更新Vx[i+j*(Parptr->xsz+1)],负值
            if(p0-p1==-1),往东流动,更新Vx[i+1+j*(Parptr->xsz+1)]，正值
            if(p0-p1==Parptr->xsz),往北流动,更新Vy[i+j*(Parptr->xsz+1)]，负值
            if(p0-p1==-Parptr->xsz),往南流动,更新Vx[i+(j+1)*(Parptr->xsz+1)]，正值
            */
					}
	...
	// Update H and reset Route_dH: 这里很重要，不要修改不要遗漏。
	for(j=0;j<Parptr->ysz;j++) 
	{
		for(i=0;i<Parptr->xsz;i++) 
		{
