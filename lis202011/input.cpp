...
void LoadDEM(Fnames *Fnameptr,States *Statesptr,Pars *Parptr,Arrays *Arrptr,int *verbose)
{
  ...
  fscanf(fp,"%s %i",dum,&Parptr->xsz);//列数
  fscanf(fp,"%s %i",dum,&Parptr->ysz);//行数
  fscanf(fp,"%s %lf",dum, &Parptr->blx);//左下角x
  fscanf(fp,"%s %lf",dum, &Parptr->bly);//左下角y
  fscanf(fp,"%s %lf",dum, &Parptr->dx);//栅格单元大小，单位m

  // 计算栅格大小的平方根用于后续计算
  Parptr->dx_sqrt = sqrt((double) Parptr->dx); //sqrt now for later use in flooplain calcs - small speed increase

  Parptr->dy=Parptr->dx;//dy=dx
  Parptr->dA=Parptr->dx*Parptr->dy;//计算栅格单元面积
  Parptr->tlx=Parptr->blx;//左上角x
  Parptr->tly=Parptr->bly+Parptr->ysz*Parptr->dy;//左上角y
  fscanf(fp,"%s %lf",dum,&no_data_value);//空值代表值
  ...
  //开辟一维水深数组内存空间，并用Arrptr->H指向该内存空间，初始化每个值为0.0
  ...
  //加载DEM数据，空值处DEM值为1*10^10
  for(j=0;j<Parptr->ysz;j++) for(i=0;i<Parptr->xsz;i++)
                             {
                               fscanf(fp,"%lf",Arrptr->DEM+i+j*Parptr->xsz);
                               //不懂为什么下面要强转成int型 可能从文件中读取double型有误差？因为double型有误差！！ 不可以直接和9999.0进行等于比较。
                               if((int)Arrptr->DEM[i+j*Parptr->xsz]==no_data_value) Arrptr->DEM[i+j*Parptr->xsz]= INFINIT;
                             }
  fclose(fp);
...
void LoadBCs(Fnames *Fnameptr,States *Statesptr,Pars *Parptr,BoundCs *BCptr,Arrays *Arrptr,int *verbose)
{
  ...
  // POINT SOURCE STUFF 点 这里浪费了好多内存空间
  ...
  while(!feof(fp))
  {
    ...
    fscanf(fp,"%s",buff);
    if(feof(fp)) break;
    if(buff[0]=='N')//获取domain北边界的固定边界条件
    {
      ...
      BCi1=(int)((start-Parptr->blx)/Parptr->dy);//获取起点数组索引
      BCi2=(int)((finish-Parptr->blx)/Parptr->dy);//获取终点数组索引
      if(BCi1>BCi2) {tmpi=BCi1;BCi1=BCi2;BCi2=tmpi;}//如果起点大于终点，交换
      BCi2--;side='N';//这里BCi2的自减操作是为了保证domain四个角只属于一条边
    }
    ...
    if(buff[0]=='W')
    {
      ...
      BCi1=(int)(2*Parptr->xsz+2*Parptr->ysz-(Parptr->tly-start)/Parptr->dy);//计算此边界起点在边界栅格一维数组中的索引
      BCi2=(int)(2*Parptr->xsz+2*Parptr->ysz-(Parptr->tly-finish)/Parptr->dy);//计算此边界终点在边界栅格一维数组中的索引
      if(BCi1>BCi2) {tmpi=BCi1;BCi1=BCi2;BCi2=tmpi;}
      BCi2--;side='W';//西侧的栅格是存储在一位数组最后，BCi2--保证了下标不会大于等于domain（2*列数+2*行数）
    }
    ...
    BC_tmp = -1;
    sscanf(buff2,"%s%lf",buff,&BC_tmp);//buff2是坐标后的字符串，buff是边界条件类型（倒数第二列），BC_tmp是输入值（最后一列）

    // If a FREE surface boundary condition
    if((!strcmp(buff,"FREE")||!strcmp(buff,"free"))&&BCi1>-1)//BCi1大于-1都代表是边的边界条件，"Free"代表使用坡度作为边界条件，识别号为1
    {
      // if BC_tmp is -1 there is no slope specifed... use local slope from elevation model (origional mehod)
      if(BC_tmp < -0.999) // ie -1 (done like this as double)使用局部坡度计算
      {
      ...
      for(i=BCi1;i<=BCi2;i++)
      {
        ...
        if(Statesptr->adaptive_ts==ON || Statesptr->qlim==ON)
        {
          ...
          else BCptr->BC_Val[i] = sqrt(BC_tmp); //对自定义坡度取平方根 要是小于0咋整？？sqrt of user specified slope for diffusive version (jcn)
        }
        else
        {
          BCptr->BC_Val[i] = BC_tmp; //对提供的参数什么也不做 user specified slope or -1(for local slope) for accelleration or any other version (jcn)
        }
      }
    }
    // Read in fixed values of H for boundary
    else if((!strcmp(buff,"HFIX")||!strcmp(buff,"hfix")) &&BCi1>-1)//边界固定水深，识别码2
    {
    ...
    else if((!strcmp(buff,"QFIX")||!strcmp(buff,"qfix")) &&BCi1>-1)//边界固定流量 识别码4
    {
      sscanf(buff2,"%s%lf",buff,&BC_tmp);
    ...
    else if((!strcmp(buff,"QVAR")||!strcmp(buff,"qvar"))&&BCi1>-1)//边界动态流量 识别码5 该边界上每个小栅格都使用同一个文件
    {
      //fscanf(fp,"%s",buff);
    ...
    else if((!strcmp(buff,"HVAR")||!strcmp(buff,"hvar"))&&BCi1>-1)//边界动态水深 识别码3 该边界上每个小栅格都使用同一个文件
    {
      //fscanf(fp,"%s",buff);
      sscanf(buff2,"%s%s",buff3,buff);
      if(*verbose==ON)
    ...
    // Fixed/Varying values/names for point sources 点边界条件
    ...
    else if((!strcmp(buff,"HFIX")||!strcmp(buff,"hfix"))&&BCptr->xpi[pi]>-1&&pi>-1)//这里的判断条件是不是有问题啊？？pi可能早就大于-1了//但由于边型的边界条件已经在此之前筛选了，在这不会有问题
    { //isPointSource=1;
      //fscanf(fp,"%lf",&BC_tmp);
      sscanf(buff2,"%s%s",buff3,buff);
      if(*verbose==ON) printf("HFIX at point [%lf,%lf] %lf\n",px,py,BC_tmp);
      BCptr->PS_Val[pi]=BC_tmp;
      BCptr->PS_Ident[pi]=2;//点HFIX，识别号2
    }
    else if((!strcmp(buff,"QFIX")||!strcmp(buff,"qfix"))&&BCptr->xpi[pi]>-1&&pi>-1)
    {
      ...
      BCptr->PS_Val[pi]=BC_tmp;
      BCptr->PS_Ident[pi]=4;//点QFIX，识别号4 我们的模型应该走了这个语句块
    }
    else if((!strcmp(buff,"QVAR")||!strcmp(buff,"qvar"))&&BCptr->xpi[pi]>-1&&pi>-1)
    {
      ...
      strcpy(BCptr->PS_Name+pi*80,buff);//记录bdy文件名
      BCptr->PS_Ident[pi]=5;//点QVAR，识别号5
    }
    else if((!strcmp(buff,"HVAR")||!strcmp(buff,"hvar"))&&BCptr->xpi[pi]>-1&&pi>-1)
    {
      ...
      BCptr->PS_Ident[pi]=3;//点HVAR，识别号3
    }
    else if((!strcmp(buff,"FREE")||!strcmp(buff,"free"))&&BCptr->xpi[pi]>-1&&pi>-1&&Statesptr->SGC==ON)
    {
      ...
      BCptr->PS_Val[pi]=BC_tmp;
      BCptr->PS_Ident[pi]=6;//点FREE，识别号6
    }
    else
    {
  ...
  //看不懂下面在干嘛 好像内存也没释放 //好像是在想根据点源个数来重新复制一份较小的数组，但BCptr下的多个指针原来指向的内存空间没有释放
  if(pi>-1)
  {
    pi++;
    new_xpi=new int[pi];
...
void LoadBCVar(Fnames *Fnameptr,States *Statesptr,Pars *Parptr,BoundCs *BCptr,ChannelSegmentType *ChannelSegments,Arrays *Arrptr, vector<ChannelSegmentType> *ChannelSegmentsVecPtr, int *verbose)
{
  ...
  numBCs=2*Parptr->xsz+2*Parptr->ysz;
  BCptr->BCVarlist=new double*[numBCs];//数组的数组，即二维数组，每个double*元素可以代表一个数组，
  ...
  while(!feof(fp))
  {
    ...
    if(nbdy==0){//过滤注释行放在循环外面不好吗？放里面每次循环都要判断
      do{
        buff[j]=fgetc(fp);
        if(feof(fp)) break;
      } while(buff[j++]!='\n');
    ...
    //以下是匹配文件中的bdy文件名和哪种边界条件匹配 是边界的，还是河流的，还是点的
    peterpointer=NULL;//用来指向二维的、每行长度不一的数组的每一行第一个元素（因为ndata可能不同）
    for(i=0;i<numBCs;i++)
    {
      if(!strcmp(buff,(BCptr->BC_Name+i*80)))
      {
        BCptr->BC_Val[i]=nbdy;//int转double，BC_Val记录文件名在par.bdyfilename文件中的索引，不记录值
        peterpointer=BCptr->BCVarlist;//指向匹配的边界条件文件列表
      }
    }
    ...
    fscanf(fp,"%i%s",&ndata,units);//读取数据个数，以及时间单位
    //据此par文件中的bdyfilename对应的文件格式应该是:
    /*----------
    ...
    */
    ...
    if(peterpointer==NULL)
    {
      ...
      continue;//跳过本条记录，那如果下一行扫描的是数据，接下来会多次重复用数据去匹配文件名！所以直接报错吧？？
    }
  ...
  for(i=0;i<numBCs;i++) if((BCptr->BC_Ident[i]==5 || BCptr->BC_Ident[i]==3) && BCptr->BC_Val[i]<0)//前面代表QVAR和HVAR，后面代表BC_Val[i]没匹配到
                        {
                          printf("WARNING: bdy %s in bci file not found in bdy file - ignored.\n",BCptr->BC_Name+i*80);
                          for(j=0;j<numBCs;j++) if(!strcmp(BCptr->BC_Name+i*80,BCptr->BC_Name+j*80)) BCptr->BC_Ident[j]=0;//0代表不操作
                        }
...
void LoadRain(Fnames *Fnameptr,Arrays *Arrptr, int *verbose)
{
  ...
  if (*verbose == ON) {
	  ...
	  for (i = 0; i < ndata; i++) {
		  printf("%f s\t%f mm/h\n", Arrptr->rain[i * 2 + 1], Arrptr->rain[i * 2] * 1000 * 3600);
	  }
  }
  return;
}
