...
{//检查floodplain函数会不会导致Qx、Vx的右侧一列以及Vx、Qy的最下一行会全为0
  int i,j;
  ...
  // 处理耦合时间步长
  /*TmpTstep = getSWMMTstep(DW, Solverptr->InitTstep);*/
  //if (Solverptr->t >= Parptr->SaveTotal); {
	 // cout << "耦合步长:" << TmpTstep << endl;
  //}
  /*Solverptr->Tstep = getmin(TmpTstep, Solverptr->Tstep);*/
  TmpTstep = Solverptr->InitTstep; // 这个变量服务于adapt_ts 或 qlim 计算解法，本质上就等于Solverptr->initTstep

//#pragma omp parallel for private(overflow)
//  for (int i = 0; i < CouplePoint::count; i++)
//  {
//	  overflow = getOverflow(QPoints[i].nIndex);
//	  QPoints[i].updateStatus(Solverptr->LastTstep, overflow); // 上一次迭代的计算步长
//	  QPoints[i].updateDischarge(Solverptr->Tstep); // 当前的计算步长
//	  QPoints[i].updatePointDepth(Solverptr->Tstep);
//	  QPoints[i].setInflow();
//  }
...
#pragma omp parallel for private( i, h0, h1, hptr0,qptr,wiptr,TSptr,ThreadTS) 
  for(j=0;j<Parptr->ysz;j++)
  {
    ...
    qptr=Arrptr->Qx+j*(Parptr->xsz+1)+1;//Qx[0,1],Qx[1,1],Qx[2,1]...每行第二列Qx
    wiptr=Arrptr->Weir_Identx+j*(Parptr->xsz+1)+1;
	// initialise thread time step for openMP
	ThreadTS = TmpTstep;
	TSptr = &ThreadTS;
    for(i=0;i<Parptr->xsz-1;i++)//注意这里的xsz-1
    {
      h0=*hptr0;
      h1=*(hptr0+1);
      *qptr=0.0;
      ...
      if(h0>Solverptr->DepthThresh || h1>Solverptr->DepthThresh)
      {
		...
		else if(Statesptr->acceleration==ON) *qptr=CalcFPQxAcc(i,j,Statesptr, Parptr, Solverptr, Arrptr);//计算出该栅格的Qx
