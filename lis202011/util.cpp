...
void DryCheck(Pars *Parptr, Solver *Solverptr,Arrays *Arrptr)
{
  ...
  // 马铮测试
//#pragma omp parallel for private( i,hptr,q1,q2,q3,q4,cv,WDweight,dV, z)
//  for (j = 0; j < Parptr->ysz; j++) {
//	  hptr = Arrptr->H + j * Parptr->xsz;//每行第一个H
//	  z = Arrptr->DEM + j * Parptr->xsz;
//	  for (i = 0; i < Parptr->xsz; i++, z++, hptr++) {
//		  if (*z != 1e10) {
//			  q1 = Arrptr->Qx + j * (Parptr->xsz + 1) + i;//西侧流量，流入为正
//			  q2 = Arrptr->Qx + j * (Parptr->xsz + 1) + i + 1;//东侧流量，流出为正
//			  q3 = Arrptr->Qy + j * (Parptr->xsz + 1) + i;//北侧流量，流入为正
//			  q4 = Arrptr->Qy + (j + 1)*(Parptr->xsz + 1) + i;//南侧流量，流出为正
//			  dV = Solverptr->Tstep*(*q1 - *q2 + *q3 - *q4);//dV为Tstep内流入的总流量，"+"代表净流入，"-"代表净流出
//			  cv = *hptr*Parptr->dA;//单元格内水量
//			  if (cv + dV < 0 && (q1 <= 0 && q2 >= 0 && q3 <= 0 && q4 >= 0)) {
...
#pragma omp parallel for private( j,pH0,por0,dAPor,p0) reduction( + : vol )
  ...
  {
	p0 = i+j*Parptr->xsz;
    if(Statesptr->porosity==ON)//由于缺少porosity文件的说明书 忽略此if语句块
    {
    ...
    {
      if(Arrptr->ChanMask[p0]==-1) vol+=Arrptr->H[p0]*Parptr->dA;//计算区域domain内的总容积
    }
  }
...
// 根据栅格索引判断是否该点为空值栅格
int isEmptyDEMGrid(int row, int col) {
	return isEqual(Arrptr->DEM[col + row * Parptr->xsz], INFINIT);
}
// 根据坐标判断是否该点为空值栅格
int isEmptyDEMGrid(double x, double y) {
	int row, col;
	col = (x - Parptr->blx) / Parptr->dx;
	row = (Parptr->tly - y) / Parptr->dy;
	return isEqual(Arrptr->DEM[col + row * Parptr->xsz], INFINIT);
}
