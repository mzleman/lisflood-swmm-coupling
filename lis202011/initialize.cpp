...
extern "C" int init(int argc, const char *argv[]) {
  ...
  *verbose = OFF;
  ...
  /*default resrootname*/
  strcpy(Fnameptr->resrootname,"out");//将保存目录名初始化为out

  //print version
  printversion();//执行一次打印版本说明的函数
  ...
  for(i=1;i<argc;i++) if(!strcmp(argv[i],"-v")) *verbose=ON;//verbose意思是信息模式、冗长模式，开启后会打印模型运行中一些提示信息
  for(i=1;i<argc;i++) if(!strcmp(argv[i],"-debug")) Statesptr->debugmode=ON;//strcmp返回0表示字符串相等，打开调试模式
  ...
  // TF: allow results root to be determined by commandline//命令行重置结果数据文件名（前缀）
  ...
  if(Statesptr->out_dir==ON)
  {
    if (fexist(Fnameptr->dirrootname)==0) // check if it doesn't exist
    {
      ...
      sprintf(tmp_sys_com,"%s%s","mkdir ",Fnameptr->dirrootname);//生成创建目录的命令字符串
      system(tmp_sys_com);//执行命令，创建目录
    }
  ...
  //ascii文件头是6行字符串，包括行数、列数 左下角坐标、无值信息等等
  ...
  LoadDEM(Fnameptr,Statesptr,Parptr,Arrptr,verbose);//读取DEM数据函数
  ...
  LoadBCs(Fnameptr,Statesptr,Parptr,BCptr,Arrptr,verbose);//加载bci文件 并加载bdy文件名
  LoadBCVar(Fnameptr,Statesptr,Parptr,BCptr,CSTypePtr,Arrptr,ChannelSegmentsVecPtr,verbose);//基于上一步操作，加载bdy文件
  ...
  //stage output file 初始化stagefile关键字下的*.stage文件（监测点水深）
  ...
  //velocity output file  初始化stagefile关键字下的*.velocity文件（监测点流速）
  ...
  //velocity output file 初始化par文件中gaugefilename关键字下的*.discharge文件（虚拟流量计）
  ...
  //start simulation
  time(&Solverptr->time_start);//返回1900年1月1日至当前的时间，单位s。同时也赋给作为参数的指针（p）所指向的实体
...
{
  Solverptr->vol1=DomainVol(Statesptr,Parptr,&(ChannelSegmentsVecPtr->front()),Arrptr,ChannelSegmentsVecPtr);//计算初始区域内总容积
...
{
  printf("***************************\n");
  printf(" LISFLOOD-FP version %d.%d.%d\n", LISFLOODVersionMajor, LISFLOODVersionMinor,LISFLOODVersionInc);
  printf("***************************\n\n");
