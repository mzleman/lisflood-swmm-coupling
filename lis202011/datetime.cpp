...
{
	int i, j;
	i = isLeapYear(year);//判断是否是闰年 1为闰年 0为非闰年
	if ((year >= 1)
		&& (year <= 9999)
		&& (month >= 1)
		&& (month <= 12)
	...
	{
		for (j = 0; j < month - 1; j++) day += DaysPerMonth[i][j];//DaysPerMonth[i][j] 是二维数组 保存了两组数据 分别为闰年和非闰年
		i = year - 1;
...

{
	...
	divMod(secs, 60, &mins, s);//第三个参数为总分钟数 第四个参数为秒数
	divMod(mins, 60, h, m);//第三个参数为分总小时数 第四个参数
	if (*h > 23) *h = 0;
}
...
//
{
	...
	char month[4];
	char sep1, sep2;
	*d = -DateDelta;//693594是公元0年1月1日到1900年1月1日一共有多少天
	...
	if (*d == -DateDelta) return 0;//没有进入if循环 失败
	else return 1;
}
...
int checkEventTime(char* start_date, char* start_time, char* end_date, char* end_time) 
//
//	校验配置文件中是否设定了模拟事件的开始时间以及结束时间
//
{
	if (start_date[0] == '\0' || start_time[0] == '\0' || end_date[0] == '\0' || end_time[0] == '\0') {
		return -1;
	}
	return 0;
}

double getSimTime(char* start_date_str, char* start_time_str, char* end_date_str, char* end_time_str) 
//
//	计算模拟时间的总时间
//	返回值单位为 秒
//
{
	...
	datetime_strToDate(start_date_str, &startDate);
	datetime_strToDate(end_date_str, &endDate);
	datetime_strToTime(start_time_str, &startTime);
	datetime_strToTime(end_time_str, &endTime);
	return datetime_timeDiff(endDate + endTime, startDate + startTime);
