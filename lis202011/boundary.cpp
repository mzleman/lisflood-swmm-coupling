...
void BCs(States *Statesptr,Pars *Parptr,Solver *Solverptr,BoundCs *BCptr,ChannelSegmentType *ChannelSegments,Arrays *Arrptr)
{
  ...
  for(BCi=0;BCi<numBCs;BCi++) 
  {
    if (BCptr->BC_Ident[BCi]!=0 || Statesptr->Roe==ON) // do nothing if closed boundary and not the Roe solver
    {
      ...
      //标记东侧边界 E edge
      else if(BCi>=Parptr->xsz && BCi<=Parptr->xsz+Parptr->ysz-1)//对的
      {  
      ...
      // FREE boundary  //freeboundary可能可以帮我们解决边界合并的问题，全部理想化为自由边界，根据local slope进行流量进出计算
      ...
      {	
        ...
        if(!(Arrptr->ChanMask[p0]!=-1)) //非河流点，条件是： !(Arrptr->ChanMask[p0]!=-1)  秀你马呢？？我人晕了
        {
		      if(Statesptr->acceleration==ON || Statesptr->Roe==ON) // 这里好像有问题 跟下面的elseif有出入。Use semi-implicit accceleration formulation
		      {
            ...
            //下面这一步没有考虑输入的Sf的方向问题啊
            //*SF/fabs(SF)
            //单宽流量，由于没考虑坡度的正负，导致单宽流量永远大于0，即往内部输入流量
			      *qoldptr=sign*(fabs(q0)+fabs(g*Solverptr->Tstep*hflow*Sf))/(1+g*Solverptr->Tstep*hflow*fn*fn*fabs(q0)/(pow(hflow,(10./3.))));
			      *qptr=*qoldptr*Parptr->dx; // potntially unnessasary?? check for repetition in UpdateQs (jcn) 就是没必要！updateQs会重复这步操作
		      }
          ...
          // adaptive_ts==on代表adaptive解法。经过origional lisflood-fp boundary方式的计算，继续完成adaptive解法的相关操作
          ...
          // qlim==on代表flow-limited解法。经过origional lisflood-fp boundary方式的计算，继续完成flow-limited解法的相关操作
        ...
        //若channelmask!=-1，是河流边界 则设Q为0
        else *qptr=0.0;

        //以下部分不知道他在干嘛，我觉得他是不知道自己哪里错了在瞎折腾。就是因为没考虑SF的正负问题
	      if(dir==1 && sign==-1 && *qptr>0) *qptr=0.0;//北侧，Q向下，则重置Q为0
	      if(dir==1 && sign==1 && *qptr<0) *qptr=0.0;//南侧，Q向上，则重置Q为0
      ...
      if(BCptr->BC_Ident[BCi]==3 && Arrptr->H[p0]>Solverptr->DepthThresh)
      {
    	  ...
    	  h0=InterpBC(BCptr->BCVarlist[(int)BCptr->BC_Val[BCi]],Solverptr->t);//这里的h0为水位数据，HVAR输入的是水位
      ...
      { 
    	  *qptr=-sign*InterpBC(BCptr->BCVarlist[(int)BCptr->BC_Val[BCi]],Solverptr->t)*Parptr->dx;
    	  hflow=getmax(Arrptr->H[p0],h0-Arrptr->DEM[p0]);//好像是一个BUG。h0没有初始化，而且QVAR用不到hflow
      ...
      // close boundary in Roe version
        ...
        {
          //以上两个if没必要分开写，直接改成if(Statesptr->Roe == ON && BCptr->BC_Ident[BCi]==0 && Arrptr->H[p0]>Solverptr->DepthThresh)
...
#pragma omp section
 ...
 BCptr->Qin = qina + qinb + qinc + qind;
  ...
  // Point sources
  BCptr->Qin+=BCptr->Qpoint;/*这里是有问题的,Qpoint可正可负，这是由bci文件中的点源边界条件决定的。
    改进:
        在updateH中计算qPointIn和qPointOut
        Qin+=qPointIn;
        Qout+=qPointOut
  */
